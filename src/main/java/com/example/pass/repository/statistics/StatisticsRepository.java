...
public interface StatisticsRepository extends JpaRepository<StatisticsEntity, Integer> {
    ...
    @Query(value = "SELECT new com.example.pass.repository.statistics.AggregatedStatistics(s.statisticsAt, SUM(s.allCount), SUM(s.attendCount), SUM(s.cancelledCount))" +
